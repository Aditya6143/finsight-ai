"""
feed_fetcher.py — News Ingestion Layer for FinSight AI
...
Fix 1: extract_query_keywords() pulls topic terms from a user question so
        that Google News is queried for the right subject automatically even
        when the user did not pre-fill the sidebar keyword field.
...
# Common English stop-words stripped when auto-extracting query keywords
_STOP_WORDS = {
...
def extract_query_keywords(question: str, max_keywords: int = 3) -> str | None:
    ...
    Strategy:
      1. Tokenise the question into words.
      2. Drop single-character tokens and common stop-words.
      3. Keep capitalised words (proper nouns / named entities) first, then
         words longer than 4 characters (substantive terms).
    ...
    kw = " ".join(chosen)
    print(f"[finsight] Auto-extracted query keywords: {kw!r}")
    return kw
...
def fetch_article_urls(
    ...
    if keyword and keyword.strip():
        ...
        print(f"[finsight] Google News (sidebar: {keyword!r}) -> {len(fetched)} articles")
        all_urls.extend(fetched)

    # --- Fix 1: auto-extracted query keyword -> Google News ---
    # Only fetch if it differs from the sidebar keyword to avoid a duplicate call
    if query_keyword and query_keyword.strip():
        if not keyword or query_keyword.strip().lower() != keyword.strip().lower():
            ...
            fetched = _parse_feed(google_url, max_articles=max_per_feed)
            print(
                f"[finsight] Google News (auto: {query_keyword!r}) -> {len(fetched)} articles"
            )
            all_urls.extend(fetched)
